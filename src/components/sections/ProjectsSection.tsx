        ...
        <div ref={projectsRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project) => (
            <AnimatedCard key={project.title} className="project-card overflow-hidden group cursor-pointer">
              ...
              <div className="p-4">
                <h3 className="text-lg font-light drop-shadow-[0_0_20px_rgba(255,255,255,0.8)]">{project.title}</h3>
                <p className="text-sm text-muted-foreground drop-shadow-[0_0_15px_rgba(255,255,255,0.6)]">{project.category}</p>
              </div>
            </AnimatedCard>
          ))}
        </div>
